...
function run(args: string[]): {success: boolean; output: string; error: string} {
...
export function getAllNotes(): string[] {
  return getAllItems().filter(i => !i.isFolder).map(i => i.path);
}

// Fuzzy search with metadata - folders sorted to top
export function searchWithMeta(query: string): {path: string; isFolder: boolean}[] {
...
export function print(noteName: string): string {
  ...
  // Search for the note
  const found = search(noteName).find(n =>
    n === noteName ||
    n === `${noteName}.md` ||
    n.endsWith(`/${noteName}`) ||
...
export function copyPath(noteName: string): {success: boolean; path: string; error: string} {
...
export function openInClaude(itemName: string): {success: boolean; path: string; error: string} {
...
export function daily(options?: {open?: boolean}): {success: boolean; path: string; error: string} {
...
export function move(from: string, to: string): {success: boolean; error: string} {
