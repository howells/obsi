  ...
  if (mode === 'processing') {
    return (
      <Box flexDirection="column" gap={1}>
        <Box gap={1}>
          <Text bold color="blue">Daily</Text>
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      {preview && (
        ...
        >
          {preview.split('\n').map((line, i) => (
            <Text key={i} dimColor={line.startsWith('---') || line.startsWith('tags:') || line.startsWith('created:')}>
              {line || ' '}
            </Text>
          ))}
        </Box>
        ...
        <Box flexDirection="column" marginTop={1}>
          {menuItems.map((item, i) => (
            <Box key={item.key}>
              ...
              <Text dimColor> [</Text>
              <Text color="cyan">{item.key === 'escape' ? 'esc' : item.key}</Text>
              <Text dimColor>] </Text>
